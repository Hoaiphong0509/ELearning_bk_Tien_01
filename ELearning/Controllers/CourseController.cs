...
namespace ELearning.Controllers
{
    public class CourseController : Controller
    {
        ...
        [HttpPost]
        public JsonResult Rate([FromForm] int courseId, string point)
        {
            point = point.Replace(".", ",");
            var temp = Request.Form;
            if (CookiesManage.Logined())
            {
                if (courseId >= 1 && RateKey.Any(Double.Parse(point)))
                {
                    using (var unitofwork = new UnitOfWork(new ELearningDbContext()))
                    {
                        var user = CookiesManage.GetUser();
                        ...
                        if (course != null)
                        {
                            ...
                            if (rating != null)
                            {
                                rating.Point = Double.Parse(point);
